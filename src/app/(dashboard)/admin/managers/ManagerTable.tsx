...
import { rejectManagerAction, approveManagerAction } from '../actions';
import { X, Phone, Mail, CheckCircle } from 'lucide-react';

export default function ManagerTable({ managers }: { managers: any[] }) {
  ...
  if (!managers || managers.length === 0) {
    return (
      <div className="rounded-2xl border border-slate-800 bg-slate-900/30 p-8 text-center text-slate-400">
        You do not have any active managers yet.
      </div>
    );
  }
        ...
        <table className="w-full text-left text-sm text-slate-300">
          ...
          <tbody className="divide-y divide-slate-800">
            {managers.map((manager) => (
              <tr key={manager.id} className={`transition-colors hover:bg-slate-800/50 ${manager.status === 'rejected' ? 'opacity-75' : ''}`}>
                <td className="px-6 py-4">
                  <div className="font-medium text-white">{manager.first_name} {manager.last_name}</div>
                  <div className="text-xs text-slate-500 capitalize">{manager.role}</div>
                </td>
                ...
                <td className="px-6 py-4 text-right">
                  {manager.status === 'approved' ? (
                    <button
                      onClick={() => handleSuspend(manager.id, `${manager.first_name} ${manager.last_name}`)}
                    ...
                    >
                      <X className="h-3 w-3" />
                      {loadingId === manager.id ? 'Wait...' : 'Suspend'}
                    </button>
                  ) : (
                    <button
                      onClick={() => handleUnblock(manager.id, `${manager.first_name} ${manager.last_name}`)}
                      disabled={loadingId === manager.id}
                    ...
                    >
                      ...
                      {loadingId === manager.id ? 'Wait...' : 'Unblock'}
                    </button>
                  )}
                </td>
              </tr>
